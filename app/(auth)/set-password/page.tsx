    ...
    const processInviteOrSession = async () => {
      try {
        ...
        if (tokenHash && type === "invite") {
          console.log(
            "[SetPassword] Found query parameter token, verifying..."
          );
          ...
          if (error) {
            ...
            setError(
              "Invalid or expired invite link. Please request a new one from your administrator."
            );
          ...
          if (accessToken && refreshToken) {
            ...
            if (error) {
              if (
                ...
                if (session) {
                  console.log(
                    "[SetPassword] Hash token already used, but session exists"
                  );
                  setError("");
                } else {
                  setError(
                    "This invite link has expired or has already been used. Please request a new invitation from your administrator."
                  );
                }
              } else {
        ...
        console.log(
          "[SetPassword] No tokens, checking for session from callback..."
        );
        ...
        while (retries > 0 && !session) {
          ...
          if (!session) {
            console.log(
              `[SetPassword] No session yet, retrying... (${retries} attempts left)`
            );
        ...
        if (!session) {
          ...
          setError(
            "This page requires an active invite link. If your invite link has expired, please request a new one from your administrator."
          );
          setTimeout(() => router.push("/login"), 3000);
        } else {
