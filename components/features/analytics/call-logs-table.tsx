                        ...
                        calls.map((call) => (
                            <TableRow key={call.call_id}>
                                <TableCell className="font-medium">
                                    {call.start_time ? format(new Date(call.start_time), 'MMM d, yyyy h:mm a') : '-'}
                                </TableCell>
                                <TableCell>{call.duration_seconds}s</TableCell>
                                <TableCell>
                                    <Badge variant={call.handoff_success ? 'default' : 'secondary'}>
                                ...
                                <TableCell>
                                    <Badge variant="outline" className={
                                        call.lead_quality_score === 'High' ? 'border-green-500 text-green-500' :
                                            call.lead_quality_score === 'Medium' ? 'border-yellow-500 text-yellow-500' :
                                                'border-gray-500 text-gray-500'
                                    }>
                                        {call.lead_quality_score || 'N/A'}
                                    </Badge>
                                </TableCell>
                                <TableCell className="max-w-[200px] truncate" title={call.primary_churn_reason}>
