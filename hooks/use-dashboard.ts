...
export function useDashboard(filters: DashboardFilters = {}) {
    ...
    const { data, isLoading, error } = useQuery({
        ...
        queryFn: async () => {
            ...
            const [unitsRes, bookingsRes, callsRes, facilitiesRes] = await Promise.all([
                supabase.from('storage_units').select('*'),
                supabase.from('bookings').select('*, storage_units(unit_number, facility, price, size)'),
                supabase.from('call_analytics')
                    .select('call_id, start_time, duration_seconds, handoff_success, csat_score, lead_quality_score, created_at, cost_credits, llm_charge, call_charge, competitor_mention, requested_move_in_window')
                    .order('start_time', { ascending: false }),
                supabase.from('storage_units').select('facility').not('facility', 'is', null)
            ])
            ...
            if (filters.facility && filters.facility !== 'all') {
                ...
                bookings = bookings.filter((b: BookingWithUnit) => b.storage_units?.facility === filters.facility)
            }
            ...
            if (dateFilterStart && dateFilterEnd) {
                allCalls = allCalls.filter((call: CallAnalytics) => {
                    ...
                    return isWithinInterval(callDate, { start: dateFilterStart!, end: dateFilterEnd! })
                })
            }
            ...
            const availableUnits = units.filter((u: Unit) => u.status === 'Available').length
            const occupiedUnits = units.filter((u: Unit) => u.status === 'Unavailable').length
            const occupancyRate = totalUnits > 0 ? ((occupiedUnits / totalUnits) * 100).toFixed(1) : '0.0'

            // Bookings
            const totalBookings = bookings.length
            const pendingBookings = bookings.filter((b: { status: string }) => b.status.toLowerCase() === 'pending').length
            const approvedBookings = bookings.filter((b: { status: string }) => b.status.toLowerCase() === 'approved').length
            ...
            const totalCredits = allCalls.reduce((sum: number, call: CallAnalytics) => sum + (call.cost_credits || 0), 0)
            const totalLLMCredits = allCalls.reduce((sum: number, call: CallAnalytics) => sum + (call.llm_charge || 0), 0)
            const totalCallCredits = allCalls.reduce((sum: number, call: CallAnalytics) => sum + (call.call_charge || 0), 0)
            ...
            allCalls.forEach((call: CallAnalytics) => {
                ...
                if (score !== null) {
                    totalCsat += score
                    csatCount++
                    csatDistribution[score.toString() as keyof typeof csatDistribution]++
                }
            })
            ...
            const costPerBooking = approvedBookings > 0
            ...
            const highQualityLeads = allCalls.filter((c: CallAnalytics) => c.lead_quality_score === 'High').length
            const mediumQualityLeads = allCalls.filter((c: CallAnalytics) => c.lead_quality_score === 'Medium').length
            const lowQualityLeads = allCalls.filter((c: CallAnalytics) => c.lead_quality_score === 'Low').length

            // Lead Quality Rates
            const highQualityRate = totalCalls > 0 ? ((highQualityLeads / totalCalls) * 100).toFixed(1) : '0.0'

            // Conversion by Lead Quality
            const highQualityConversion = highQualityLeads > 0
            ...
            const performanceByDay = new Map<string, { revenue: number, bookings: number, calls: number, credits: number }>()

            for (let i = 29; i >= 0; i--) {
                const date = subDays(new Date(), i)
                const dateStr = format(date, 'MMM dd')
                performanceByDay.set(dateStr, { revenue: 0, bookings: 0, calls: 0, credits: 0 })
            }
            ...
            const roiTrendData = Array.from(performanceByDay.entries()).map(([date, data]) => {
                const dayCost = data.credits * 0.001
                const dayROI = dayCost > 0 ? ((data.revenue - dayCost) / dayCost * 100) : 0
                return {
                    date,
            ...
            const conversionFunnelData = [
                ...
                { stage: 'High Quality Leads', count: highQualityLeads, rate: totalCalls > 0 ? (highQualityLeads / totalCalls * 100) : 0 },
                { stage: 'Successful Handoffs', count: successfulHandoffs, rate: totalCalls > 0 ? (successfulHandoffs / totalCalls * 100) : 0 },
                { stage: 'Approved Bookings', count: approvedBookings, rate: totalCalls > 0 ? (approvedBookings / totalCalls * 100) : 0 },
                { stage: 'Active Leases', count: activeBookings.length, rate: totalCalls > 0 ? (activeBookings.length / totalCalls * 100) : 0 },
            ]
            ...
            const leadQualityPerformanceData = [
                {
                    ...
                    conversions: allCalls.filter((c: CallAnalytics) => c.lead_quality_score === 'High' && c.handoff_success).length,
                    rate: highQualityLeads > 0 ? (allCalls.filter((c: CallAnalytics) => c.lead_quality_score === 'High' && c.handoff_success).length / highQualityLeads * 100) : 0
                },
                {
                    quality: 'Medium',
                    count: mediumQualityLeads,
                    conversions: allCalls.filter((c: CallAnalytics) => c.lead_quality_score === 'Medium' && c.handoff_success).length,
                    rate: mediumQualityLeads > 0 ? (allCalls.filter((c: CallAnalytics) => c.lead_quality_score === 'Medium' && c.handoff_success).length / mediumQualityLeads * 100) : 0
                },
                {
                    quality: 'Low',
                    count: lowQualityLeads,
                    conversions: allCalls.filter((c: CallAnalytics) => c.lead_quality_score === 'Low' && c.handoff_success).length,
                    rate: lowQualityLeads > 0 ? (allCalls.filter((c: CallAnalytics) => c.lead_quality_score === 'Low' && c.handoff_success).length / lowQualityLeads * 100) : 0
                }
            ]
            ...
            const volumeMap = new Map<string, { date: string, calls: number, bookings: number, avgDuration: number, totalDuration: number }>()
            const daysToShow = 30
            ...
            for (let i = daysToShow - 1; i >= 0; i--) {
                ...
                const dateStr = format(date, 'MMM dd')
                volumeMap.set(dateStr, { date: dateStr, calls: 0, bookings: 0, avgDuration: 0, totalDuration: 0 })
            }
            ...
            // 2. CSAT Chart Data
            const csatChartData = Object.entries(csatDistribution).map(([score, count]) => ({
                score: `${score} Star`,
                count
            }))
            ...
            allCalls.forEach((call: CallAnalytics) => {
                if (call.lead_quality_score && ['High', 'Medium', 'Low'].includes(call.lead_quality_score)) {
                    leadQualityMap[call.lead_quality_score as keyof typeof leadQualityMap]++
                }
            })
            const leadQualityData = Object.entries(leadQualityMap).map(([name, value]) => ({ name, value }))

            // 4. Unit status distribution
            const unitStatusDistribution = [
                { name: 'Available', value: availableUnits },
                { name: 'Pending Apps', value: units.filter((u: Unit) => u.status === 'Submitted').length },
                { name: 'Occupied', value: units.filter((u: Unit) => u.status === 'Unavailable').length }
            ]

            // 5. Facility-Level Analytics
            const facilityBreakdown = facilities.map(facility => {
                const facilityUnits = units.filter((u: Unit) => u.facility === facility)
                const facilityBookings = bookings.filter((b: BookingWithUnit) => b.storage_units?.facility === facility)

                return {
                    facility,
                    totalUnits: facilityUnits.length,
                    availableUnits: facilityUnits.filter((u: Unit) => u.status === 'Available').length,
                    occupiedUnits: facilityUnits.filter((u: Unit) => u.status === 'Unavailable').length,
                    bookings: facilityBookings.length,
                    occupancyRate: facilityUnits.length > 0
                        ? ((facilityUnits.filter((u: Unit) => u.status === 'Unavailable').length / facilityUnits.length) * 100).toFixed(1)
                        : '0.0'
                }
            })
            ...
            allCalls.forEach((call: CallAnalytics) => {
                if (call.competitor_mention && call.competitor_mention !== 'None') {
                    competitorMentions[call.competitor_mention] = (competitorMentions[call.competitor_mention] || 0) + 1
                }
            })
            ...
            allCalls.forEach((call: CallAnalytics) => {
                if (call.requested_move_in_window && call.requested_move_in_window !== 'Not Specified') {
                    moveInWindows[call.requested_move_in_window] = (moveInWindows[call.requested_move_in_window] || 0) + 1
                }
            })
            ...
            const creditBreakdownData = callVolumeData.map((day) => {
                ...
                const llmCredits = dateCalls.reduce((sum: number, call: CallAnalytics) => sum + (call.llm_charge || 0), 0)
                const voiceCredits = dateCalls.reduce((sum: number, call: CallAnalytics) => sum + (call.call_charge || 0), 0)
