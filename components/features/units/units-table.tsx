...
export function UnitsTable({
  ...
  const [selectedUnitForBookings, setSelectedUnitForBookings] =
    useState<Unit | null>(null);
  const [showBulkDeleteDialog, setShowBulkDeleteDialog] = useState(false);
  const rowRefs = useRef<Record<number, HTMLTableRowElement | null>>({});
  const { deleteUnits, updateUnitsStatus } = useUnits();
  ...
  const filteredUnits = units.filter((unit) => {
    ...
    const matchesFacility =
      facilityFilter === "all" || unit.facility === facilityFilter;
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case "Available":
        return "bg-[#2C9143] text-white border-[#2C9143]";
      case "Submitted":
        return "bg-[#C4642F] text-white border-[#C4642F]";
      case "Unavailable":
  ...
  const handleBulkStatusUpdate = (status: string) => {
    ...
    // Get unique facilities from selected units
    const selectedUnits = units.filter((u) => selectedUnitIds.includes(u.id));
    const uniqueFacilities = Array.from(
      new Set(selectedUnits.map((u) => u.facility))
    );

    updateUnitsStatus.mutate(
      { ids: selectedUnitIds, status, facilities: uniqueFacilities },
      {
        onSuccess: () => {
          toast.success(`Updated ${selectedUnitIds.length} units to ${status}`);
          setSelectedUnitIds([]);
        },
        onError: (error) => {
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <ConfirmationDialog
            ...
            onConfirm={() => {
              deleteUnits.mutate(selectedUnitIds, {
                onSuccess: () => {
                  toast.success(`Deleted ${selectedUnitIds.length} units`);
                  setSelectedUnitIds([]);
                },
                onError: (error) => {
            ...
            title="Delete Units"
            description={`Are you sure you want to delete ${selectedUnitIds.length} units? This action cannot be undone.`}
            confirmText="Delete"
            variant="danger"
          />
          <div className="h-4 w-px bg-border mx-2" />
          ...
          <Button
            variant="outline"
            size="sm"
            onClick={() => exportToExcel(units, "Units", "Units")}
              ...
              filteredUnits.map((unit) => (
                <TableRow
                  ...
                  className={`transition-colors duration-500 ${
                    selectedUnitIds.includes(unit.id) ? "bg-muted/50" : ""
                  }`}
                >
                  <TableCell>
                    <Checkbox
                    ...
                    <div className="flex justify-end gap-2">
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          <DropdownMenuItem
                            onClick={() => onStatusUpdate(unit.id, "Available")}
                          >
                            Mark Available
                          </DropdownMenuItem>
                          <DropdownMenuItem
                            onClick={() =>
                              onStatusUpdate(unit.id, "Unavailable")
                            }
                          >
                            Mark Unavailable
                          </DropdownMenuItem>
