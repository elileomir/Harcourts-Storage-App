...
import { User, Bot, Wrench, Award, Target, CheckCircle2, FileText, X, Terminal } from 'lucide-react'
...
function getCSATBadge(score: string | null) {
    if (!score) return { variant: 'secondary' as const, label: 'N/A', className: 'bg-gray-100 text-mid-grey' }
                ...
                <div className="bg-white border-t-4 border-harcourts-blue px-8 py-6 relative shadow-[0_2px_8px_rgba(0,0,0,0.1)] z-10">
                    <SheetHeader className="space-y-1">
                        <div className="flex items-center justify-between">
                            <SheetTitle className="text-navy text-2xl font-bold tracking-tight">Call Details</SheetTitle>
                        ...
                        <SheetDescription className="text-mid-grey text-base font-normal">
                            {new Date(call.start_time).toLocaleString()} • Duration: {formatDuration(call.duration_seconds)}
                        </SheetDescription>
                    </SheetHeader >
                </div >
                        ...
                        {call.transcript_summary && (
                            <Card className="border-none shadow-[0_2px_8px_rgba(0,0,0,0.1)] overflow-hidden rounded-[4px]">
                                ...
                                <CardContent className="bg-white">
                                    <p className="text-sm text-navy leading-relaxed">{call.transcript_summary}</p>
                                </CardContent>
                            </Card>
                        )}
                            ...
                            <CardContent className="bg-white">
                                <div className="flex flex-col sm:flex-row gap-8">
                                    <div className="space-y-3 flex-1">
                                        <label className="text-xs font-bold text-mid-grey uppercase tracking-wider">CSAT Score</label>
                                        <div className="flex">
                                            <Badge className={`${csatBadge.className} text-sm px-4 py-1.5 rounded-[4px] shadow-none border-0 font-semibold`}>
                                                {csatBadge.label}
                                            </Badge>
                                        </div>
                                    </div>
                                    <div className="space-y-3 flex-1">
                                        <label className="text-xs font-bold text-mid-grey uppercase tracking-wider">Lead Quality</label>
                            ...
                            <Card className="border-none shadow-[0_2px_8px_rgba(0,0,0,0.1)] overflow-hidden rounded-[4px]">
                                ...
                                <CardContent className="bg-white space-y-6">
                                    {call.evaluation_rationale.brand_alignment && (
                                        <div className="space-y-3">
                                            <div className="flex items-center justify-between pb-2">
                                                <h4 className="font-bold text-sm text-navy">Brand Alignment</h4>
                                                <Badge className={`rounded-[4px] font-semibold ${call.evaluation_rationale.brand_alignment.result === 'success' ? 'bg-success hover:bg-success/90' : 'bg-destructive hover:bg-destructive/90'}`}>
                                    ...
                                    {call.evaluation_rationale.handoff_success && (
                                        <div className="space-y-3">
                                            <div className="flex items-center justify-between pb-2">
                                                <h4 className="font-bold text-sm text-navy">Handoff Success</h4>
                                                <Badge className={`rounded-[4px] font-semibold ${call.evaluation_rationale.handoff_success.result === 'success' ? 'bg-success hover:bg-success/90' : 'bg-destructive hover:bg-destructive/90'}`}>
                                    ...
                                    {call.evaluation_rationale.compliance_check && (
                                        <div className="space-y-3">
                                            <div className="flex items-center justify-between pb-2">
                                                <h4 className="font-bold text-sm text-navy">Compliance Check</h4>
                                                <Badge className={`rounded-[4px] font-semibold ${call.evaluation_rationale.compliance_check.result === 'success' ? 'bg-success hover:bg-success/90' : 'bg-destructive hover:bg-destructive/90'}`}>
                                                ...
                                                >
                                                    ...
                                                    <div className="flex-1 min-w-0 space-y-2">
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            <span className="text-xs text-mid-grey font-mono">{formatTime(turn.time_in_call_secs)}</span>
                                                        </div>
                                                                ...
                                                                {turn.tool_calls!.map((tool, toolIndex) => (
                                                                    ...
                                                                    >
                                                                        ...
                                                                        <span className="font-mono text-xs font-semibold text-navy">
                                                                            Called: {tool.tool_name || tool.name || 'Unknown Tool'}
                                                                        </span>
                                                                    </div>
                                                                ))}
                                                            </div>
                                                            ...
                                                            <div className="space-y-2 mt-2">
                                                                {turn.tool_results!.map((result, resultIndex) => (
                                                                    ...
                                                                    >
                                                                        ...
                                                                        <span className="font-mono text-xs font-semibold text-navy">
                                                                            Result: {result.tool_name || result.name || 'System Action'}
