      ...
      .on(
        ...
        async (payload) => {
          if (payload.eventType === "INSERT") {
            ...
            const {
              data: { user },
            } = await supabase.auth.admin.getUserById(newProfile.id);
            if (user) {
              setUsers((prev) => [
                ...prev,
                {
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              <strong>{userToDelete?.email}</strong>? This will remove the user
              from both authentication and the profiles table. This action
              cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
