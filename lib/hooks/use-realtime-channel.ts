    ...
    useEffect(() => {
        ...
        const setupChannel = () => {
            ...
            if (channelRef.current) {
                console.log(`[${channelName}] Removing existing channel before reconnect`)
            ...
            channel.on('system', {}, (payload) => {
                ...
                if (payload.status === 'TIMED_OUT') {
                    console.warn(`[${channelName}] Connection timed out, reconnecting in 3s...`)
                    ...
                    console.error(`[${channelName}] ❌ Persistent channel error - check Supabase Realtime configuration`)
                    // Don't reconnect on CHANNEL_ERROR - it's likely a configuration issue
                }
            })
        ...
        const handleVisibilityChange = () => {
            if (document.visibilityState === 'visible' && channelRef.current && !isUnmountedRef.current) {
                const state = channelRef.current.state
                console.log(`[${channelName}] Tab became visible, channel state:`, state)

                // Reconnect if channel is closed or errored
                if (state === 'closed' || state === 'errored') {
                    console.log(`[${channelName}] 🔄 Tab visible with dead channel, reconnecting...`)
                    setupChannel()
                }
            }
        }
